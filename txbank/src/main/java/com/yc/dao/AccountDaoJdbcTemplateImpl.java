...
public class AccountDaoJdbcTemplateImpl implements AccountDao {
    ...
    public void update(int accountid, double money) {
        this.jdbcTemplate.update("update accounts set balance = balance +? where accountid = ?",
                money + "", accountid + "");
    }
    ...
    public List<Account> findAll() {
        //模板模式  while(rs.next()) { Object obj = rowMapper.map(rs, i); }
        List<Account> list = jdbcTemplate.query("select * from accounts", (rs, rowNum) -> {
    ...
    public Account findById(int accountid) {
        Account a = jdbcTemplate.queryForObject("select * from accounts where accountid = ?", (rs, rowNum) -> {
            Account account = new Account();
            account.setAccountid(rs.getInt(1));
            account.setMoney(rs.getDouble(2));
            return account;
